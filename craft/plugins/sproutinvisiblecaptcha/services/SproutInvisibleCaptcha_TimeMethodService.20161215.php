...
class SproutInvisibleCaptcha_TimeMethodService extends BaseApplicationComponent implements SproutInvisibleCaptcha_MethodInterfaceService
{
	...
	public function verifySubmission()
	{
		...
		$arr = array('viagra', 'href=', 'url=', 'link=', 'http:', '.com');
		$notspam = true;
		
		foreach ($_POST['fields'] as $key => $value) {
			if ($key == 'message') {
				$message = $_POST['fields'][$key];

				foreach ($arr as $arr_value) 
					if (strpos($message, $arr_value)) $notspam = false;
			}
		}
		...
		{
			if (!$notspam)
				SproutInvisibleCaptchaPlugin::log("A form submission failed because the message contains spam words: " . $message, LogLevel::Info, true);
			else 
